...
def main():
    ...
    parser.add_argument(
        "-f", "--force", action="store_true", help="forces override of vspace file creation"
    )
            ...
            # user wants to randomly sample a normal/gaussian distribution
                ...
                if mode == 1:
                    ...
                    if "min_cutoff" in vars() and "max_cutoff" not in vars():
                        ...
                        for ll in np.arange(len(array)):
                            array[ll] = np.random.normal(
                                loc=np.float(values[0]), scale=np.float(values[1]), size=1
                            )
                else:
                    raise IOError(
    ...
    if numvars == 0:
        for i in range(fnum):
            ...
            )  # bookkeeping (has option been found in file?)
